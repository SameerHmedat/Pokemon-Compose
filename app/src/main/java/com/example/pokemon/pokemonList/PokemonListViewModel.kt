...
class PokemonListViewModel @Inject constructor(
    ...
    private var isSearchStarting = true // just about to start a search
            ...
            when(result) {
                is Resource.Success -> {
                    ...
                    val pokedexEntries = result.data.results.mapIndexed { index, entry ->
                        ...
                        val formattedName = entry.name.replaceFirstChar { ch ->
                            if (ch.isLowerCase()) ch.titlecase(Locale.ROOT) else ch.toString()
                        }

                        PokedexListEntry(formattedName, url, number.toInt())
                    }
